...
import { ListContainer, ListItem } from '../styles/style';
...
const List = ({ setAuthenticated }) => {
  ...
  const [deleteTask] = useMutation(DELETE_TASK, {
    update: (cache, response) => {
      if (response.data) {
        toast('Task Deleted!', { type: 'warning' });
  ...
  return (
    <ListContainer>
      ...
      {data && data.tasks.length === 0 && <ListItem>Add task above!</ListItem>}
      {console.log(data)}
      {data &&
        data.tasks
          .filter((t) => {
              ...
              <div className="actions">
                {startedTasks.includes(task.id) ? (
                  <IconButton
                    ...
                    onClick={() => {
                      setStartedTasks(
                        startedTasks.filter((s) => s !== task.id)
                      );
